...
class Game:
    ...
    def draw_power_up_time(self):
        # Ask if player has a powerUp
        if self.player.has_power_up:
            time_to_show = round((self.player.time_with_power - pygame.time.get_ticks()) / 1000, 2)
            if time_to_show > 0:
                self.menu.draw(self.screen, f"{self.player.power_up_type.upper()} is enabled for {time_to_show} seconds.", 540, 50, (255, 255, 255))
            else:
